...
Next, the for() loop used to loop every char from `original_virus` variable and SUB with the `kolona_genome` array.
Thus we can put it into [cyberchef](https://gchq.github.io/CyberChef/#recipe=SUB(%7B'option':'Hex','string':''%7D)SUB(%7B'option':'Hex','string':''%7D))
